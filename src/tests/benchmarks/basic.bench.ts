...
function benchmarkDeepNesting() {
    ...
    console.log(`Result Pattern Nesting: ${comparison.resultB.operationsPerSecond.toFixed(2)} ops/sec`);
...
function benchmarkErrorHandlingOverhead() {
    ...
    console.log(`Result pattern error handling: ${resultPatternResult.operationsPerSecond.toFixed(2)} ops/sec`);
    console.log(`Result pattern is ${percentDifference.toFixed(2)}% slower than direct error handling`);
...
function benchmarkMemoryUsage() {
    ...
    function createResultObjects(count: number) {
        ...
        for (let i = 0; i < count; i++) {
            results.push(
                i % 2 === 0
                    ? success({ id: i, value: Math.random() })
    ...
    const plainObjectsBenchmark = runBenchmark(
        () => createObjects(10_000),
        {
            iterations: 10,
            warmupRuns: 1,
            gcBetweenRuns: true,
    ...
    const resultObjectsBenchmark = runBenchmark(
        () => createResultObjects(10_000),
        {
            iterations: 10,
            warmupRuns: 1,
            gcBetweenRuns: true,
    ...
    const plainObjectMemoryPerItem = plainObjectsBenchmark.memoryDelta.heapUsed / (10_000 * plainObjectsBenchmark.iterations);
    const resultObjectMemoryPerItem = resultObjectsBenchmark.memoryDelta.heapUsed / (10_000 * resultObjectsBenchmark.iterations);
    ...
    console.log(`Result objects use ${((resultObjectMemoryPerItem / plainObjectMemoryPerItem) - 1) * 100}% more memory`);
...
export async function runAllBenchmarks() {
    ...
    const reporters = [
        { name: 'Function Call Overhead: Direct Function Calling', results: functionCallResults.resultA },
    ...
    console.log('Benchmark suite completed! Report saved to ./benchmark-reports/result-pattern-analysis.html');
