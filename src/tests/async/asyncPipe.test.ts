...
describe("(ASYNC) `asyncPipe` function", () => {
  ...
  test("should handle initial error Result", async () => {
    ...
    const result = await asyncPipe(initialError, async (x) => {
      functionCalled = true;
      return x * 2;
    });
  ...
  test("should convert non-Error exceptions to Error objects", async () => {
    // String exception
    const stringResult = await asyncPipe(5, async () => {
      throw "String error";
    });
    ...
    // Object exception
    const objectResult = await asyncPipe(5, async () => {
  ...
  test("should handle rejected promises in transformation functions", async () => {
    const result = await asyncPipe(5, async () => Promise.reject(new Error("Promise rejected")));
