...
describe("(CORE) `chain` function", () => {
  ...
  test("should pass through original error results without calling the function", () => {
    ...
    const chainedResult = chain(errorResult, (_n) => {
      functionCalled = true;
  ...
  test("should handle different error types", () => {
    ...
    const dbResult = chain(initialResult, (_n) =>
      error(new DatabaseError(500, "Database connection failed")),
    );
    ...
    const combinedResult = chain(validationResult, (_n) =>
      error(new DatabaseError(500, "Database connection failed")),
    );
  ...
  test("should handle multiple chains", () => {
    ...
    const successfulChain = chain(
      chain(chain(success("42"), parseNumber), double),
      requirePositive,
    );
    ...
    const failStep1 = chain(
      chain(chain(success("not-a-number"), parseNumber), double),
      requirePositive,
    );

    assert.strictEqual(failStep1.status, "error");
    assert.strictEqual(
      (failStep1.error as Error).message,
      "Failed to parse 'not-a-number' as number",
    );

    // Failing at last step
    const failStep3 = chain(chain(chain(success("-5"), parseNumber), double), requirePositive);
