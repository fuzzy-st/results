...
export function validationPipelineExample() {
  ...
  function validateEmail(input: string): Result<string, Error> {
    ...
    return emailRegex.test(input) ? success(input) : error(new Error("Invalid email format"));
  }
  ...
  function validateUserEmail(email: string): Result<string, Error> {
    return pipe(email, validateNonEmpty, validateMinLength(5), validateEmail, sanitizeEmail);
  }
  ...
  function validateUserData(input: UserInput): Result<ValidatedUser, Error> {
    // Name validation
    const nameResult = pipe(input.name, validateNonEmpty, validateMinLength(2));
...
export function apiRequestPipelineExample() {
  ...
  function authenticateRequest(request: ApiRequest): Result<ApiRequest, Error> {
    ...
    return isAuthenticated ? success(request) : error(new Error("Unauthorized"));
  }
  ...
  function processResponse<T, R>(
    ...
    try {
      ...
      return error(err instanceof Error ? err : new Error("Error processing response"));
    }
  }

  // Create API request pipeline
  function apiRequest<T, R>(request: ApiRequest, processor: (data: T) => R): Result<R, Error> {
    return pipe(
      request,
      validateRequest,
      authenticateRequest,
  ...
  const userResult = apiRequest<User, string>(
    userRequest,
...
export function railwayProgrammingExample() {
  ...
  function validatePositive(x: number): Result<number, Error> {
    return x > 0 ? success(x) : error(new Error(`Value ${x} is not positive`));
  }

  function requireEven(x: number): Result<number, Error> {
    return x % 2 === 0 ? success(x) : error(new Error(`Value ${x} is not even`));
  }
