...
export function basicTapErrorExamples() {
  ...
  const tappedSuccess = tapError(successResult, (_err) => {
...
export function practicalErrorHandlingExample() {
  ...
  function logError<T, E extends Error>(context: string, result: Result<T, E>): Result<T, E> {
    return tapError(result, (err) => {
      errorLog.push({
    ...
    return tapError(result, (err) => {
      ...
      if (existingRetry) {
        if (existingRetry.retryCount < maxRetries) {
          ...
          console.log(
            `Incremented retry for ${operation}: attempt ${existingRetry.retryCount}/${maxRetries}`,
          );
        } else {
  ...
  // Example usage
  const failedOp1 = markForRetry("send-email", 3, error(new Error("SMTP connection failed")));

  const failedOp2 = markForRetry("process-payment", 2, error(new Error("Gateway timeout")));
