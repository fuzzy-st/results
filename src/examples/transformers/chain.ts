...
export function basicChainExamples() {
  ...
  function validatePositive(n: number): Result<number, Error> {
    return n > 0 ? success(n) : error(new Error("Number must be positive"));
  }
...
export function dataParsingExample() {
  ...
  function parseNumber(str: string): Result<number, Error> {
    ...
    return Number.isNaN(num) ? error(new Error(`Invalid number format: "${str}"`)) : success(num);
  }
  ...
  const validChain = chain(chain(chain(validInput, parseNumber), validateAge), categorizeAge);
  console.log("Valid Chain Result:", validChain);

  console.log("\nProcessing invalid format: 'not-a-number'");
  const invalidFormatChain = chain(
    chain(chain(invalidFormat, parseNumber), validateAge),
    categorizeAge,
  );
  ...
  const negativeAgeChain = chain(
    chain(chain(negativeAge, parseNumber), validateAge),
    categorizeAge,
  );
  ...
  const unrealisticAgeChain = chain(
    chain(chain(unrealisticAge, parseNumber), validateAge),
    categorizeAge,
  );
...
export function databaseOperationsExample() {
  ...
  const firstPostResult = chain(postsResult, (posts) => {
    ...
    return firstPost ? success(firstPost) : error(new Error(`Post ${aliceFirstPostId} not found`));
  });
  ...
  const nonExistentUserChain = chain(findUser(nonExistentId), findUserPosts);
...
export function formValidationExample() {
  ...
  const validFormResult = chain(
    chain(chain(chain(success(validForm), validateUsername), validateEmail), validatePassword),
    validateAge,
  );
  ...
  const invalidUsernameResult = chain(
    chain(
      chain(chain(success(invalidUsername), validateUsername), validateEmail),
      validatePassword,
    ),
    validateAge,
  );
  ...
  const invalidEmailResult = chain(
    chain(chain(chain(success(invalidEmail), validateUsername), validateEmail), validatePassword),
    validateAge,
  );
  ...
  const invalidPasswordResult = chain(
    chain(
      chain(chain(success(invalidPassword), validateUsername), validateEmail),
      validatePassword,
    ),
    validateAge,
  );
  ...
  const invalidAgeResult = chain(
    chain(chain(chain(success(invalidAge), validateUsername), validateEmail), validatePassword),
    validateAge,
  );
