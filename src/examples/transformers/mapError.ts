...
export function basicMapErrorExamples() {
    ...
    const enhancedErrorResult = mapError(errorResult, err =>
    ...
    const mappedSuccessResult = mapError(successResult, err =>
        new Error("This won't happen")
    );
    console.log("Original Success Result:", successResult);
    console.log("Mapped Success Result:", mappedSuccessResult);

    return { errorResult, enhancedErrorResult, stringErrorResult, successResult, mappedSuccessResult };
}
...
export function errorClassificationExamples() {
    ...
    return {
        networkErrorResult, normalizedNetworkError,
        validationErrorResult, normalizedValidationError,
        authErrorResult, normalizedAuthError,
        internalErrorResult, normalizedInternalError,
        randomErrorResult, normalizedRandomError
    };
}
...
export function apiErrorHandlingExample() {
    ...
    return {
        notFoundResponse, transformedNotFound,
        unauthorizedResponse, transformedUnauthorized,
        serverErrorResponse, transformedServerError,
        successResponse, transformedSuccess
    };
}

// Run all examples
function runMapErrorExamples() {
...
function normalizeError(err: Error) {
    ...
    } else if (err.message.includes("auth") || err.message.includes("login") || err.message.includes("permission")) {
        return {
            category: "authentication",
            message: "Authentication failed. Please sign in again.",
            originalError: err
        };
    } else if (err.name === "InternalError" || err.message.includes("internal")) {
        return {
            category: "internal",
