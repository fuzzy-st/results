...
export async function errorClassificationExample() {
  ...
  async function translateError(err: Error): Promise<UserFacingError> {
    ...
    return new UserFacingError(userMessage, errorCode, severity, err);
  }
  ...
  console.log(
    "User message:",
    isError(translatedNetworkError) && translatedNetworkError.error.userMessage,
  );
  ...
  console.log(
    "User message:",
    isError(translatedValidationError) && translatedValidationError.error.userMessage,
  );
  ...
  function handleApiError(apiError: ApiError) {
    ...
    switch (Math.floor(apiError.statusCode / 100)) {
      ...
      case 5:
        ...
        console.log(
          `Would send to error monitoring: ${JSON.stringify({
            type: "api_error",
...
// Run all examples
async function _runAsyncMapErrorExamples() {
