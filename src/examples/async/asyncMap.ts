...
export async function basicAsyncMapExample() {
  ...
  const finalResult = await asyncMap(doubledResult, asyncAddOne);
...
export async function userDataEnrichmentExample() {
  ...
  function findUser(id: number) {
    ...
    return user ? success(user) : error(new Error(`User with ID ${id} not found`));
  }
  ...
  if (isSuccess(userResult)) {
    ...
    if (isSuccess(enrichedResult)) {
      ...
      console.log(
        `User ${enrichedResult.data.name} has ${enrichedResult.data.posts.length} posts.`,
      );
    }
  }
  ...
  if (isSuccess(noPostsUserResult)) {
    ...
    if (isSuccess(enrichedNoPostsResult)) {
      ...
      console.log(
        `User ${enrichedNoPostsResult.data.name} has ${enrichedNoPostsResult.data.posts.length} posts.`,
      );
    }
  }
...
export async function errorHandlingExample() {
  ...
  async function fetchWithRejection(_url: string): Promise<object> {
    // Simulate API delay
    await new Promise((resolve) => setTimeout(resolve, 150));
