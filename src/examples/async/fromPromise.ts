...
export async function apiRequestExample() {
  ...
  const validUserResult = await fromPromise(fetchUser(42), apiErrorTransformer);

  console.log("Valid User Result:", validUserResult);

  console.log("\nFetching invalid user (ID: 999)...");
  const invalidUserResult = await fromPromise(fetchUser(999), apiErrorTransformer);
...
// Run all examples
async function _runFromPromisesExamples() {
