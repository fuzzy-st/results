  ...
  async function displayUser(userId: string) {
    ...
    if (isError(result)) {
      ...
      if (result.error instanceof ValidationError) {
        ...
        return `Invalid input: ${Object.entries(result.error.fields)
          .map(([k, v]) => `${k}: ${v}`)
          .join(", ")}`;
      }
...
export function stateMachineExample() {
  ...
  class StateMachine {
    ...
    setError(message: string, retryCount = 0) {
      this.state = success({
  ...
  function handlePayment(amount: number, method: "card" | "bank") {
    ...
    const message = match(result, {
      ...
      error: (err) => {
        ...
        if (err instanceof FraudDetectedError) {
  ...
  function processFile(content: string, filename: string) {
    ...
    if (isError(result)) {
      if (result.error instanceof ParseError) {
        ...
        console.log(
          `Format mismatch: expected ${result.error.expected}, got ${result.error.received}`,
        );
    ...
    if (isSuccess(result)) {
      ...
      switch (parsed.format) {
        ...
        case "csv":
          console.log(
            `CSV file with ${parsed.headers.length} columns and ${parsed.rows.length} rows`,
          );
          break;
        case "xml":
