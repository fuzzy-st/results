 ...
 */
type UnwrapResultValue<T> = T extends Result<infer V, any>
    ? V
 ...
 */
type UnwrapResultError<T> = T extends Result<any, infer E>
    ? E
...
export function asyncPipe<T, E>(initialValue: Result<T, E> | Promise<Result<T, E>>): Promise<Result<T, E>>;
...
export async function asyncPipe(
    ...
    try {
        ...
        return error(
            e instanceof Error
                ? e
                : new Error(`Unexpected error in asyncPipe: ${String(e)}`)
        );
    }
}